...
<div class="mx-auto max-w-7xl">
    <div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('inventory.export') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700">
        ...
        <div class="border-b border-slate-200 dark:border-slate-700 p-4">
            <form action="{{ route('inventory.index') }}" method="GET" class="flex gap-2 max-w-md">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search Item Code or Description..." class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 pl-9 pr-4 py-2 text-sm text-slate-900 dark:text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">
                </div>
                ...
                @if(request('search'))
                    <a href="{{ route('inventory.index') }}" class="rounded-xl px-4 py-2 text-sm font-medium text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">Clear</a>
                @endif
            </form>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    @forelse ($items as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                            ...
                            <td class="px-6 py-4">{{ $item->quantity_acquired }}</td>
                            <td class="px-6 py-4">{{ $item->quantity_distributed }}</td>
                            <td class="px-6 py-4 font-medium {{ $item->quantity_remaining <= 0 ? 'text-red-500' : 'text-emerald-600 dark:text-emerald-400' }}">{{ $item->quantity_remaining }}</td>
                            <td class="px-6 py-4">{{ $item->supplier }}</td>
                            <td class="px-6 py-4">{{ $item->brand }}</td>
                            <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ $item->remarks ?? '-' }}</td>
                            <td class="px-6 py-4 text-right">
                                <button type="button" onclick="openEditModal({{ $item->toJson() }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 mr-3">Edit</button>
                                <button type="button" onclick="openDeleteModal({{ $item->id }}, '{{ addslashes($item->name) }}')" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
