...
<div class="relative space-y-6">
    ...
    @if(session('success'))
        <div class="mb-4 rounded border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 px-4 py-3 text-emerald-700 dark:text-emerald-400">
            {{ session('success') }}
        </div>
    @endif
                    ...
                    <li class="flex items-start justify-between gap-4 rounded-xl border px-3 py-4
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @endif
                                <a href="{{ $link }}" class="text-sm {{ $isUnread || $isLatest ? 'font-semibold text-slate-800 dark:text-slate-100' : 'text-slate-600 dark:text-slate-300' }} hover:text-blue-600 dark:hover:text-blue-400 hover:underline">
                                    {{ $message }}
                                </a>
                            </div>
