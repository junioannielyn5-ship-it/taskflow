...
<div class="relative space-y-4 sm:space-y-6 min-h-screen p-3 sm:p-4 lg:p-6">
    ...
    <div class="relative overflow-visible rounded-2xl sm:rounded-3xl border border-white/40 bg-white/40 px-4 sm:px-8 py-5 sm:py-8 shadow-lg backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/40 mb-6 sm:mb-8 z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 sm:gap-6">
        ...
        <div class="flex w-full shrink-0 flex-wrap sm:flex-row gap-2 sm:gap-3 md:w-auto">
            <a href="{{ route('tasks.list') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow-md transition-all hover:bg-blue-700 hover:scale-105 hover:shadow-lg">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 lg:gap-6 relative z-10">
        ...
        <a href="/projects" class="relative overflow-hidden rounded-3xl border border-white/40 bg-white/60 p-6 shadow-lg backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/60 dark:shadow-slate-900/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
            <div class="relative z-10">
                ...
                <h3 class="text-slate-900 dark:text-white text-5xl font-black mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $totalProjects }}</h3>
        ...
        <a href="{{ route('tasks.list') }}" class="relative overflow-hidden rounded-3xl border border-white/40 bg-white/60 p-6 shadow-lg backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/60 dark:shadow-slate-900/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
            <div class="relative z-10">
                <div class="flex justify-between items-start mb-4">
                ...
                <h3 class="text-slate-900 dark:text-white text-5xl font-black mb-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{{ $totalTasks }}</h3>
                <div class="mt-4 h-2 w-full bg-slate-200/50 dark:bg-slate-700/50 rounded-full overflow-hidden shadow-inner">
        ...
        <a href="{{ route('tasks.list', ['status' => 'for_review']) }}" class="relative overflow-hidden rounded-3xl border border-white/40 bg-white/60 p-6 shadow-lg backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/60 dark:shadow-slate-900/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
            <div class="relative z-10">
                <div class="flex justify-between items-start mb-4">
                ...
                <h3 class="text-slate-900 dark:text-white text-5xl font-black mb-1 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{{ $pendingReview }}</h3>
                <div class="mt-4 h-2 w-full bg-slate-200/50 dark:bg-slate-700/50 rounded-full overflow-hidden shadow-inner">
        ...
        <a href="{{ route('tasks.list') }}" class="relative overflow-hidden rounded-3xl border border-white/40 bg-white/60 p-6 shadow-lg backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/60 dark:shadow-slate-900/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
            <div class="relative z-10">
                ...
                <h3 class="text-slate-900 dark:text-white text-5xl font-black mb-1 group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">{{ $overdue }}</h3>
                <div class="mt-4 h-2 w-full bg-slate-200/50 dark:bg-slate-700/50 rounded-full overflow-hidden shadow-inner">
                ...
                <div id="latest-notifications-card" class="relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/40 bg-white/60 p-4 sm:p-6 shadow-lg backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/60 dark:shadow-slate-900/50 transition-all scroll-mt-24">
                    <div class="mb-4 flex items-center justify-between gap-3">
                        ...
                        <div class="flex items-center gap-2">
                            <a href="{{ route('dashboard.export.status-overview.csv') }}" class="rounded-lg border border-slate-300 dark:border-slate-600 bg-white/50 dark:bg-slate-700/50 px-3 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-600 shadow-sm transition-all">CSV</a>
                            <a href="{{ route('dashboard.export.status-overview.pdf') }}" class="rounded-lg border border-slate-300 dark:border-slate-600 bg-white/50 dark:bg-slate-700/50 px-3 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-600 shadow-sm transition-all">PDF</a>
                        </div>
                    </div>
                    <p class="mb-3 text-sm font-medium text-slate-600 dark:text-slate-400">Done: <span id="done-percentage" class="font-extrabold text-emerald-600">{{ $donePercentage }}%</span> · In Progress: <span id="in-progress-percentage" class="font-extrabold text-blue-600">{{ $inProgressPercentage }}%</span></p>
                            ...
                            <div class="rounded-xl border-2 border-emerald-300 dark:border-emerald-500/50 overflow-hidden bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/25 dark:to-green-900/15 shadow-[0_0_10px_rgba(16,185,129,0.18)] pointer-events-none">
                                <div class="flex w-full items-center justify-between px-4 py-3">
                                    <div class="flex items-center gap-2 min-w-0">
                                        ...
                                        <span class="truncate text-sm font-bold text-emerald-800 dark:text-emerald-200">{{ $ex['name'] }}</span>
                                    </div>
                                    <div class="ml-3 flex flex-shrink-0 items-center gap-3">
                                        ...
                                        <span class="tabular-nums text-xs font-semibold text-emerald-700 dark:text-emerald-400">{{ $ex['done'] }}/{{ $ex['total'] }}</span>
                                        <div class="relative h-2 w-24 overflow-hidden rounded-full bg-emerald-100 dark:bg-emerald-900/40">
                                            <div class="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-emerald-400 to-emerald-600 shadow-[0_0_6px_rgba(16,185,129,0.5)] progress-bar" data-width="{{ $ex['percent'] }}"></div>
                                        </div>
                                        <span class="w-8 text-right text-sm font-extrabold text-emerald-600 dark:text-emerald-400">{{ $ex['percent'] }}%</span>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                                ...
                                <tbody>
                                    @foreach(($urgentTasks ?? collect())->take(8) as $task)
                                        <tr class="border-b border-slate-100 dark:border-slate-700/50 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800">
                                            ...
                                            <td class="px-2 py-1.5 text-slate-500 dark:text-slate-400 text-xs truncate">{{ Illuminate\Support\Str::limit($task->project->name ?? '-', 12) }}</td>
                                            <td class="px-2 py-1.5 {{ $task->isOverdue() ? 'font-semibold text-rose-600' : 'text-slate-600 dark:text-slate-400' }} text-xs whitespace-nowrap">{{ $task->due_date ? $task->due_date->format('m-d') : '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                                ...
                                <div class="rounded-lg border border-slate-200/50 dark:border-slate-700/50 bg-white/40 dark:bg-slate-800/40 p-2.5 hover:bg-white/60 dark:hover:bg-slate-700/60 transition-colors">
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{{ $meeting->meeting_date?->format('M d, Y') }}</p>
                                            @if($meeting->start_time)
                                                <p class="text-xs text-slate-500 dark:text-slate-400">{{ $meeting->start_time }}@if($meeting->end_time) - {{ $meeting->end_time }}@endif</p>
                                            @endif
                                        </div>
                                    </div>
                                    @if($meeting->location)
                                        <p class="mt-1.5 text-xs text-slate-500 dark:text-slate-400 truncate">📍 {{ Illuminate\Support\Str::limit($meeting->location, 20) }}</p>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="dashboard-panel relative flex h-[320px] flex-col overflow-hidden rounded-3xl dashboard-glass-panel border-none p-4 shadow-md dark:shadow-none transition-all hover:shadow-lg">
                    ...
                    <ul class="activity-timeline relative z-10 flex-1 space-y-3 overflow-y-auto pr-1" style="padding-left: 2px;">
                        @forelse(($recentActivities ?? $recentActivity) as $logIdx => $log)
                            ...
                            @endphp
                            <li class="relative flex items-start gap-3 {{ $isLatestLog ? 'rounded-lg bg-blue-50 dark:bg-blue-500/10 px-2 py-1 ring-1 ring-blue-200 dark:ring-blue-500/30' : '' }}">
                                @php
                                ...
                                <div class="-mt-1 min-w-0 flex-1">
                                    <div class="flex items-start justify-between gap-3">
                                        <p class="text-xs leading-4">
                                            ...
                                            @endif
                                            <span class="font-bold text-slate-800 dark:text-slate-200">{{ $log->actor?->name ?? 'System' }}</span>
                                            <span class="text-slate-600 dark:text-slate-400"> {{ $desc }}</span>
                                        </p>
                                        <span class="whitespace-nowrap text-xs italic text-slate-400">{{ $log->created_at?->diffForHumans() }}</span>
                                    </div>
                        ...
                        @empty
                            <li class="py-3">
                                <div class="flex flex-col items-center rounded-2xl border border-dashed border-slate-300 dark:border-slate-600 bg-white/40 dark:bg-slate-800/40 px-4 py-6 text-center">
                                    ...
                                    <a href="{{ route('tasks.create') }}" class="mt-3 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-indigo-500">Get Started</a>
                                </div>
                            </li>
                        @endforelse
                    </ul>
                ...
                <div class="dashboard-panel relative flex h-[320px] flex-col overflow-hidden rounded-3xl dashboard-glass-panel border-none p-4 shadow-md dark:shadow-none transition-all hover:shadow-lg">
                    ...
                    <ul id="latest-notifications-list" class="relative z-10 flex-1 divide-y divide-slate-200 dark:divide-slate-700 overflow-y-auto pr-1">
                        @forelse($latestNotifications as $notifIdx => $notif)
                            ...
                            <li class="flex items-start justify-between gap-2 py-2 text-xs rounded-lg px-2 {{ $isLatestNotif ? 'bg-blue-50 dark:bg-blue-500/10 ring-1 ring-blue-300 dark:ring-blue-500/40 mb-1' : '' }}" data-notification-id="{{ $notif->id }}" data-notification-link="{{ $notif->data['link'] ?? '' }}">
