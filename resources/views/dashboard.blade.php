...
<div class="relative space-y-4 min-h-screen p-3 sm:p-4 lg:p-6">
    ...
    <div class="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6 z-10">
        ...
        @can('create-project')
            <a href="{{ route('projects.create') }}" class="inline-flex items-center justify-center rounded-lg bg-blue-600 hover:bg-blue-700 px-3.5 py-1.5 text-xs font-bold text-white shadow-sm transition-all hover:-translate-y-0.5 duration-200">
    ...
    <div class="relative overflow-hidden rounded-2xl border border-slate-200/50 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 mb-6 z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div class="flex w-full shrink-0 flex-wrap sm:flex-row gap-2 md:w-auto">
            <a href="{{ route('tasks.list') }}" class="inline-flex items-center justify-center gap-1.5 rounded-lg bg-blue-600 px-3.5 py-1.5 text-xs font-bold text-white shadow-sm transition-all hover:bg-blue-700 hover:-translate-y-0.5 duration-200">
            ...
            <a href="{{ route('tasks.kanban') }}" class="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200/60 bg-white/70 px-3.5 py-1.5 text-xs font-bold text-slate-900 shadow-sm backdrop-blur-md transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/90 dark:bg-slate-900/70 dark:text-white dark:border-slate-700 dark:hover:bg-slate-900/90">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 relative z-10">
        ...
        <a href="/projects" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $totalProjects }}</h3>
        ...
        <a href="{{ route('tasks.list') }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{{ $totalTasks }}</h3>
                <div class="mt-3.5 h-1.5 w-full bg-slate-200/40 dark:bg-slate-800/40 rounded-full overflow-hidden shadow-inner">
        ...
        <a href="{{ route('tasks.list', ['status' => 'for_review']) }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{{ $pendingReview }}</h3>
                <div class="mt-3.5 h-1.5 w-full bg-slate-200/40 dark:bg-slate-800/40 rounded-full overflow-hidden shadow-inner">
        ...
        <a href="{{ route('tasks.list') }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">{{ $overdue }}</h3>
                <div class="mt-3.5 h-1.5 w-full bg-slate-200/40 dark:bg-slate-800/40 rounded-full overflow-hidden shadow-inner">
            ...
            <div class="relative overflow-hidden rounded-2xl border border-slate-200/40 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 transition-all z-10 mb-6" id="mv-progress-panel">
                ...
                @else
                    ...
                    <div id="mv-tab-content-sales" class="mv-tab-content relative z-10 space-y-2">
                        ...
                        @if(!$salesHasAny)
                            {{-- Example Sales project progress cards --}}
                            @foreach([
                            ...
                            <div class="rounded-xl border border-slate-200/40 overflow-hidden bg-slate-50/70 dark:border-slate-800 dark:bg-slate-950/70 pointer-events-none hover:-translate-y-0.5 transition-all duration-200 shadow-sm">
                                <div class="flex w-full items-center justify-between px-3.5 py-2.5">
                                    <div class="flex items-center gap-2 min-w-0">
                                        ...
                                        <span class="truncate text-xs font-bold text-emerald-800 dark:text-emerald-200">{{ $ex['name'] }}</span>
                                    </div>
                                    <div class="ml-3 flex flex-shrink-0 items-center gap-2.5">
                                        ...
                                        <span class="tabular-nums text-[10px] font-semibold text-emerald-700 dark:text-emerald-400">{{ $ex['done'] }}/{{ $ex['total'] }}</span>
                                        <div class="relative h-1.5 w-20 overflow-hidden rounded-full bg-emerald-100 dark:bg-emerald-900/40">
                                            <div class="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-emerald-400 to-emerald-600 shadow-[0_0_5px_rgba(16,185,129,0.4)] progress-bar" data-width="{{ $ex['percent'] }}"></div>
                                        </div>
                                        <span class="w-8 text-right text-[11px] font-extrabold text-emerald-600 dark:text-emerald-400">{{ $ex['percent'] }}%</span>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                                ...
                                <div class="mv-accordion rounded-xl border border-slate-200/40 bg-white/70 dark:border-slate-800 dark:bg-slate-900/70 overflow-hidden hover:-translate-y-0.5 transition-all duration-200">
                                    ...
                                    <button
                                        ...
                                        <div class="flex items-center gap-2 min-w-0">
                                            ...
                                            <span class="truncate text-xs font-semibold text-slate-800 dark:text-white">{{ $project['name'] }}</span>
                                        </div>
                                        <div class="ml-3 flex flex-shrink-0 items-center gap-2.5">
                                            ...
                                            <span class="w-8 text-right text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">{{ $project['sales']['percent'] }}%</span>
                                        ...
                                        <div class="mv-task-flow space-y-3">
                                                    ...
                                                    <div class="flex min-w-[580px] items-start justify-between gap-1">
                                                        @foreach($project['sales']['tasks'] as $ti => $task)
                                                            ...
                                                            <button type="button" class="mv-task-node relative flex flex-col items-center px-1 text-center {{ $ti === 0 ? 'is-selected' : '' }}" data-target="sales-{{ $project['id'] }}-task-{{ $task['id'] }}" onclick="mvSelectTask(this)">
                                                                ...
                                                                <span class="mt-1.5 text-[10px] font-bold text-slate-700 dark:text-slate-300">{{ $task['task_no'] ?: ('Task ' . ($ti + 1)) }}</span>
                                                                <span class="text-[9px] uppercase tracking-wide text-slate-500 font-semibold">{{ $statusLabel }}</span>
                                                            </button>
                                                        @endforeach
                                                    </div>
                                                </div>
                                            ...
                                            <div class="space-y-1.5">
                                                @foreach($project['sales']['tasks'] as $ti => $task)
                                                    ...
                                                    @endphp
                                                    <div id="sales-{{ $project['id'] }}-task-{{ $task['id'] }}" class="mv-task-detail rounded-lg border border-slate-200/40 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3 {{ $ti === 0 ? '' : 'hidden' }}">
                                                        <div class="flex items-center gap-2">
                                                            <span class="text-[10px] font-extrabold text-emerald-600 dark:text-emerald-400">{{ $task['task_no'] ?: ('Task ' . ($ti + 1)) }}</span>
                                                            <span class="truncate text-xs font-semibold text-slate-700 dark:text-slate-200">{{ $task['title'] }}</span>
                                                            <span class="ml-auto rounded-full px-2 py-0.5 text-[9px] font-bold {{ $statusBadgeClass }}">{{ ucwords(str_replace('_', ' ', $task['status'])) }}</span>
                                                        </div>
                                                        @if($task['due_date'])
                                                            <p class="mt-0.5 text-[10px] text-slate-400 font-medium">Due: {{ $task['due_date'] }}</p>
                                                        @endif
                                                    </div>
                                                @endforeach
                                            </div>
                                ...
                                <div class="mv-accordion rounded-xl border border-slate-200/40 bg-white/70 dark:border-slate-800 dark:bg-slate-900/70 overflow-hidden hover:-translate-y-0.5 transition-all duration-200">
                                    <button
                                        ...
                                        <div class="ml-3 flex flex-shrink-0 items-center gap-2.5">
                                            ...
                                            <span class="w-8 text-right text-[10px] font-semibold text-blue-600 dark:text-blue-400">{{ $project['technical']['percent'] }}%</span>
                                            <svg class="mv-chevron h-3.5 w-3.5 flex-shrink-0 text-slate-400 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                        </div>
                                    </button>
                                    <div class="mv-accordion-body hidden border-t border-slate-100/60 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/20 px-3 py-3">
                                        <div class="mv-task-flow space-y-3">
                                            ...
                                            <div class="space-y-1.5">
                                                @foreach($project['technical']['tasks'] as $ti => $task)
                                                    ...
                                                    @endphp
                                                    <div id="technical-{{ $project['id'] }}-task-{{ $task['id'] }}" class="mv-task-detail rounded-lg border border-slate-200/40 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3 {{ $ti === 0 ? '' : 'hidden' }}">
                                                        <div class="flex items-center gap-2">
                                                            <span class="text-[10px] font-extrabold text-blue-600 dark:text-blue-400">{{ $task['task_no'] ?: ('Task ' . ($ti + 1)) }}</span>
                                                            <span class="truncate text-xs font-semibold text-slate-700 dark:text-slate-200">{{ $task['title'] }}</span>
                                                            <span class="ml-auto rounded-full px-2 py-0.5 text-[9px] font-bold {{ $statusBadgeClass }}">{{ ucwords(str_replace('_', ' ', $task['status'])) }}</span>
                                                        </div>
                                                        @if($task['due_date'])
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div class="relative overflow-hidden rounded-2xl border border-slate-200/40 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 transition-all">
                    <div class="relative z-10 mb-2.5 flex items-center justify-between">
                        ...
                        <a href="{{ route('tasks.list', ['priority' => 'urgent']) }}" class="text-[11px] font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:underline">View All</a>
                    </div>
                    ...
                    @else
                        <div class="relative z-10 overflow-x-auto rounded-xl border border-slate-200/40 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 max-h-60 overflow-y-auto">
                            <table class="min-w-full text-xs">
                                ...
                                <tbody>
                                    @foreach(($urgentTasks ?? collect())->take(8) as $task)
                                        <tr class="border-b border-slate-100/50 dark:border-slate-800 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-100/30 dark:hover:bg-slate-800/30">
                                            <td class="px-3 py-1.5 font-medium text-slate-800 dark:text-slate-200 truncate">
                                                <a href="{{ route('tasks.show', $task) }}" class="text-blue-600 hover:text-blue-800 hover:underline" title="{{ $task->title }}">{{ Illuminate\Support\Str::limit($task->title, 20) }}</a>
                                            </td>
                                            <td class="px-3 py-1.5 text-slate-500 dark:text-slate-400 text-[11px] truncate">{{ Illuminate\Support\Str::limit($task->project->name ?? '-', 12) }}</td>
                                            <td class="px-3 py-1.5 {{ $task->isOverdue() ? 'font-semibold text-rose-600' : 'text-slate-600 dark:text-slate-400' }} text-[11px] whitespace-nowrap">{{ $task->due_date ? $task->due_date->format('m-d') : '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div class="relative overflow-hidden rounded-2xl border border-slate-200/40 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 transition-all">
                    <div class="relative z-10 mb-2.5 flex items-center justify-between">
                        ...
                        <a href="{{ route('meetings.index') }}" class="text-[11px] font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:underline">View All</a>
                    </div>
                                    ...
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="min-w-0 flex-1">
                                            <a href="{{ route('meetings.index') }}" class="block text-xs font-semibold text-blue-600 hover:text-blue-800 truncate" title="{{ $meeting->title }}">
                                                {{ Illuminate\Support\Str::limit($meeting->title, 24) }}
                                            </a>
                                            <p class="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400 font-medium">{{ $meeting->meeting_date?->format('M d, Y') }}</p>
                                            @if($meeting->start_time)
                                                <p class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{{ $meeting->start_time }}@if($meeting->end_time) - {{ $meeting->end_time }}@endif</p>
                                            @endif
                                        </div>
                                    </div>
                                    @if($meeting->location)
                                        <p class="mt-1 text-[11px] text-slate-500 dark:text-slate-400 truncate">📍 {{ Illuminate\Support\Str::limit($meeting->location, 20) }}</p>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="relative flex h-[280px] flex-col overflow-hidden rounded-2xl border border-slate-200/40 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 transition-all">
                    <div class="relative z-10 mb-2.5 flex items-center justify-between">
                        ...
                        <a href="{{ route('tasks.list') }}" class="text-[11px] font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:underline">View All</a>
                    </div>
                    <ul class="activity-timeline relative z-10 flex-1 space-y-2 overflow-y-auto pr-1" style="padding-left: 2px;">
                        @forelse(($recentActivities ?? $recentActivity) as $logIdx => $log)
                            @php
                            ...
                            @endphp
                            <li class="relative flex items-start gap-2.5 {{ $isLatestLog ? 'rounded-lg bg-blue-50/50 dark:bg-blue-500/5 px-2 py-1 ring-1 ring-blue-200/50 dark:ring-blue-500/20' : '' }}">
                                @php
                                    $dotColorValue = trim(str_replace('background:', '', $dotColor)) ?: '#8b5cf6';
                                @endphp
                                <div class="timeline-dot" {!! 'style="background-color:' . e($dotColorValue) . ';"' !!}></div>
                                <div class="-mt-1 min-w-0 flex-1">
                                    <div class="flex items-start justify-between gap-3">
                                        <p class="text-[11px] leading-relaxed">
                                            ...
                                            @endif
                                            <span class="font-extrabold text-slate-800 dark:text-slate-200">{{ $log->actor?->name ?? 'System' }}</span>
                                            <span class="text-slate-600 dark:text-slate-400"> {{ $desc }}</span>
                                        </p>
                                        <span class="whitespace-nowrap text-[10px] italic text-slate-400 font-medium">{{ $log->created_at?->diffForHumans() }}</span>
                                    </div>
                        ...
                        @empty
                            <li class="py-3">
                                <div class="flex flex-col items-center rounded-xl border border-dashed border-slate-200/60 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 p-4 text-center">
                                    ...
                                    <a href="{{ route('tasks.create') }}" class="mt-2.5 rounded-lg bg-indigo-600 px-3 py-1 text-[11px] font-bold text-white shadow-sm hover:bg-indigo-700 transition duration-200">Get Started</a>
                                </div>
                            </li>
                        @endforelse
                    </ul>
                ...
                <div class="relative flex h-[280px] flex-col overflow-hidden rounded-2xl border border-slate-200/40 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 transition-all">
                    ...
                    <ul id="latest-notifications-list" class="relative z-10 flex-1 divide-y divide-slate-200/50 dark:divide-slate-700/50 overflow-y-auto pr-1">
                        @forelse($latestNotifications as $notifIdx => $notif)
                            @php $isLatestNotif = $notifIdx === 0; @endphp
                            <li class="flex items-start justify-between gap-2 py-1.5 text-[11px] rounded-lg px-1.5 {{ $isLatestNotif ? 'bg-blue-50/50 dark:bg-blue-500/5 ring-1 ring-blue-300/40 dark:ring-blue-500/20 mb-1' : '' }}" data-notification-id="{{ $notif->id }}" data-notification-link="{{ $notif->data['link'] ?? '' }}">
                                <div class="flex flex-1 items-start gap-1.5 min-w-0">
                                    ...
                                    @endif
                                    <a href="{{ $notif->data['link'] ?? route('dashboard') }}" class="flex-1 {{ $isLatestNotif ? 'font-semibold text-blue-700 dark:text-blue-300 hover:text-blue-800' : 'text-slate-700 dark:text-slate-300 hover:text-blue-600' }} hover:underline">
                                        {{ Illuminate\Support\Str::limit($notif->data['message'] ?? $notif->data['body'] ?? '-', 72) }}
                                    </a>
                                </div>
