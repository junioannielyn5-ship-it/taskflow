...
<div class="relative space-y-6">
    ...
    <div class="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6 z-10">
        ...
        @can('create-project')
            <a href="{{ route('projects.create') }}" class="inline-flex items-center justify-center rounded-lg bg-blue-600 hover:bg-blue-700 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm transition-all hover:-translate-y-0.5 duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" class="mr-1.5 h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
                Create Project
            </a>
        @endcan
    </div>

    <div class="relative overflow-visible rounded-xl border border-slate-200/40 bg-white/80 px-3 py-2 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 flex flex-wrap gap-2 mb-4 z-10">
        <div class="relative inline-block" id="projects-project-picker">
            <button
                type="button"
                id="projects-project-toggle-btn"
                class="inline-flex items-center gap-1.5 rounded-lg border-0 ring-1 ring-inset ring-slate-300/40 bg-white/70 py-1 px-2.5 text-[11px] font-semibold text-slate-900 shadow-sm backdrop-blur-sm transition-all hover:bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-slate-900/55 dark:text-white dark:ring-slate-700/50 dark:hover:bg-slate-800"
            >
                <span class="text-[9px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Project:</span>
                <span class="font-bold" id="projects-project-selected-label">All Projects</span>
                <svg class="h-3 w-3 text-slate-400 ml-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
            </button>
            <div id="projects-project-dropdown" class="absolute left-0 z-30 mt-1.5 hidden min-w-[220px] rounded-lg border border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 p-1 shadow-md backdrop-blur-xl">
                ...
                @foreach(($projects ?? collect())->pluck('name')->filter()->unique()->sort()->values() as $projectName)
                    <button
                        type="button"
        ...
        <div class="relative inline-block" id="projects-company-picker">
            <button
                ...
                class="inline-flex items-center gap-1.5 rounded-lg border-0 ring-1 ring-inset ring-slate-300/40 bg-white/70 py-1 px-2.5 text-[11px] font-semibold text-slate-900 shadow-sm backdrop-blur-sm transition-all hover:bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-slate-900/55 dark:text-white dark:ring-slate-700/50 dark:hover:bg-slate-800"
            >
                <span class="text-[9px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Company:</span>
            ...
            <div id="projects-company-dropdown" class="absolute left-0 z-30 mt-1.5 hidden min-w-[220px] rounded-lg border border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 p-1 shadow-md backdrop-blur-xl">
                ...
                @foreach(($projects ?? collect())->pluck('company_name')->filter()->unique()->sort()->values() as $companyName)
                    <button
                        type="button"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 items-stretch">
        @forelse($projects as $project)
            <div class="project-card relative overflow-hidden flex flex-col h-full rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group"
                data-project-name="{{ strtolower((string) $project->name) }}"
                data-company-name="{{ strtolower((string) ($project->company_name ?? '')) }}">
                ...
                <div class="relative z-10 grid grid-cols-2 gap-x-3 gap-y-2 text-[11px]">
                    ...
                    <div class="col-span-2">
                        ...
                        <span class="font-semibold text-slate-700 dark:text-slate-200 ml-1">{{ $project->project_owner ?: '-' }}</span>
                    </div>

                    <div class="col-span-2">
                        <span class="font-bold text-slate-500 dark:text-slate-400">Description:</span>
                        <span class="font-medium text-slate-500 dark:text-slate-400 ml-1">{{ Str::limit($project->description, 80) }}</span>
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        @endphp
                        <span class="px-2 py-0.5 text-[10px] font-bold rounded-md {{ $statusClass }} ml-1">
                            {{ $statusLabel }}
                        </span>
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        @endphp
                        <span class="px-2 py-0.5 text-[10px] font-bold rounded-md {{ $roleClass }} ml-1">
                ...
                <div class="relative z-10 mt-auto pt-3">
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        <a href="{{ route('projects.show', $project) }}" class="inline-flex items-center gap-1 rounded border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-950/40 px-2 py-0.5 text-[10px] font-semibold text-blue-600 dark:text-blue-400 hover:bg-blue-100/80 dark:hover:bg-blue-900/50 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                            View
                        </a>
                        @can('create-task')
                            <a href="{{ route('tasks.create', ['project_id' => $project->id]) }}" class="inline-flex items-center gap-1 rounded border border-emerald-100 dark:border-emerald-900/50 bg-emerald-50/50 dark:bg-emerald-950/40 px-2 py-0.5 text-[10px] font-semibold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100/80 dark:hover:bg-emerald-900/50 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                Create Task
                            </a>
                        @endcan
                        @can('update', $project)
                            <a href="{{ route('projects.edit', $project) }}" class="inline-flex items-center gap-1 rounded border border-amber-100 dark:border-amber-900/50 bg-amber-50/50 dark:bg-amber-950/40 px-2 py-0.5 text-[10px] font-semibold text-amber-700 dark:text-amber-400 hover:bg-amber-100/80 dark:hover:bg-amber-900/50 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
                        ...
                        @can('delete', $project)
                            <form action="{{ route('projects.destroy', $project) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Are you sure you want to delete this project?')" class="inline-flex items-center gap-1 rounded border border-red-100 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/40 px-2 py-0.5 text-[10px] font-semibold text-red-650 dark:text-red-400 hover:bg-red-100/80 dark:hover:bg-red-900/50 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
