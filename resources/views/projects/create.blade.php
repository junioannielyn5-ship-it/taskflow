...
<div class="mx-auto max-w-3xl space-y-4">
    <div class="mb-4">
        <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Create Project</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Set up a new project space and align your team on scope and delivery.</p>
    </div>

    <form method="POST" action="{{ route('projects.store') }}" class="rounded-xl border border-slate-200/40 bg-white/90 dark:border-slate-800 dark:bg-slate-900/90 p-4 shadow-sm space-y-3.5">
        @csrf
        <div>
        ...
        <div class="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-2">
            ...
            <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-blue-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm hover:bg-blue-700 transition-all duration-200 hover:-translate-y-0.5">Create</button>
        </div>
    </form>
</div>
@endsection
