    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px; margin:0 auto; background:#ffffff; border:1px solid #e5e7eb; border-radius:12px; overflow:hidden;">
        ...
        <tr>
            <td style="padding:20px;">
                ...
                <p style="margin:0 0 8px;"><strong>{{ $task->title }}</strong></p>
                <p style="margin:0 0 8px;"><strong>Target Deadline:</strong> {{ $task->due_date ? $task->due_date->format('M d, Y') : '-' }}</p>
                <p style="margin:0 0 16px;"><strong>Status:</strong> {{ strtoupper(str_replace('_', ' ', $task->status ?? '')) }}</p>
                <a href="{{ $taskUrl }}" style="display:inline-block; background:#b91c1c; color:#ffffff; text-decoration:none; padding:10px 14px; border-radius:8px; font-size:14px;">Review Task</a>
            </td>
        </tr>
        <tr>
