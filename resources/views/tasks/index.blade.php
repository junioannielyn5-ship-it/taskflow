...
<div class="relative space-y-4 tasks-page">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 relative z-10">
        <!-- Active Projects -->
        <a href="{{ route('tasks.list') }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $activeProjectsCount ?? 0 }}</h3>
        ...
        <!-- My Tasks -->
        <a href="{{ route('tasks.list', ['assignee' => auth()->id()]) }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                <div class="flex justify-between items-center mb-3">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{{ $myTasksCount ?? 0 }}</h3>
                <div class="mt-3.5 h-1.5 w-full bg-slate-200/40 dark:bg-slate-800/40 rounded-full overflow-hidden shadow-inner">
        ...
        <!-- Pending Review -->
        <a href="{{ route('tasks.list', ['status' => 'for_review']) }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                <div class="flex justify-between items-center mb-3">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{{ $pendingReviewCount ?? 0 }}</h3>
                <div class="mt-3.5 h-1.5 w-full bg-slate-200/40 dark:bg-slate-800/40 rounded-full overflow-hidden shadow-inner">
        ...
        <!-- Overdue -->
        <a href="{{ route('tasks.list', ['overdue' => 1]) }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                <div class="flex justify-between items-center mb-3">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">{{ $overdueCount ?? 0 }}</h3>
                <div class="mt-3.5 h-1.5 w-full bg-slate-200/40 dark:bg-slate-800/40 rounded-full overflow-hidden shadow-inner">
        ...
        <!-- Done -->
        <a href="{{ route('tasks.list', ['tab' => 'done']) }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 hover:-translate-y-0.5 transition-all duration-200 group">
            <div class="relative z-10">
                <div class="flex justify-between items-center mb-3">
                ...
                <h3 class="text-slate-900 dark:text-white text-3xl font-extrabold mb-1 tracking-tight group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{{ $doneTasksCount ?? 0 }}</h3>
                <div class="mt-3.5 h-1.5 w-full bg-slate-200/40 dark:bg-slate-800/40 rounded-full overflow-hidden shadow-inner">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="relative inline-flex items-center overflow-hidden rounded-lg border border-slate-200/40 bg-white/80 p-0.5 shadow-sm dark:border-slate-800 dark:bg-slate-900/90">
            <a href="{{ route('tasks.list', array_merge(request()->except(['tab', 'done_page', 'page']), ['tab' => 'active'])) }}"
               class="relative flex items-center justify-center rounded-md px-3.5 py-1.5 text-xs font-bold transition-all duration-200 {{ $currentTab === 'active' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:bg-white/50 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-700/50 dark:hover:text-white' }}">
            ...
            <a href="{{ route('tasks.list', array_merge(request()->except(['tab', 'page']), ['tab' => 'done'])) }}"
               class="relative flex items-center justify-center rounded-md px-3.5 py-1.5 text-xs font-bold transition-all duration-200 {{ $currentTab === 'done' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-600 hover:bg-white/50 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-700/50 dark:hover:text-white' }}">
    ...
    <form method="GET" action="{{ route('tasks.list') }}" class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white/80 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 z-10">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8 items-end">
            ...
            <div>
                ...
                <input id="company" type="text" name="company" value="{{ $filters['company'] ?? '' }}" placeholder="Client..." class="block w-full rounded-lg border-0 bg-white/70 py-1.5 px-2.5 text-xs font-semibold text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300/40 backdrop-blur-sm transition-all focus:bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-slate-900/55 dark:text-white dark:ring-slate-700/50 dark:focus:bg-slate-800 dark:focus:ring-blue-500 placeholder:text-slate-400">
            </div>
            ...
            <div>
                ...
                <input id="date_received" type="date" name="date_received" value="{{ $filters['date_received'] ?? '' }}" class="block w-full rounded-lg border-0 bg-white/70 py-1.5 px-2.5 text-xs font-semibold text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300/40 backdrop-blur-sm transition-all focus:bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-slate-900/55 dark:text-white dark:ring-slate-700/50 dark:focus:bg-slate-800 dark:focus:ring-blue-500">
            </div>
        ...
        <div class="mt-4 flex items-center justify-end gap-3 border-t border-slate-200/40 pt-3 dark:border-slate-800">
            <a href="{{ route('tasks.list') }}" class="inline-flex items-center justify-center rounded-lg px-3.5 py-1.5 text-xs font-bold text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/50 dark:hover:text-white">
                Clear Filters
            </a>
    ...
    @if ($currentTab === 'active')
        ...
        @else
            <div class="mb-2 flex items-center justify-between">
                ...
                <span class="rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-bold text-blue-700 dark:bg-blue-900/45 dark:text-blue-300">{{ $tasks->total() }} task(s)</span>
            </div>
            <div class="overflow-x-auto rounded-xl border border-slate-200/40 bg-white/90 shadow-sm dark:border-slate-800 dark:bg-slate-900/90">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/30 bg-transparent">
                    @foreach ($tasks as $index => $task)
                        ...
                        @endphp
                        <tr class="{{ $rowAccentClass }} {{ $isDelayed ? 'is-overdue-row bg-red-50/10 dark:bg-red-950/15' : 'bg-transparent' }} hover:bg-slate-50/50 dark:hover:bg-slate-800/55 transition-colors duration-200">
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                                <a href="{{ route('tasks.show', $task) }}" class="font-bold text-blue-600 hover:underline">{{ $taskNumber }}</a>
                            </td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                            ...
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $projectOwner }}</td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $task->team_in_charge ?: '-' }}</td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                                @if($task->task_process)
                                    <span class="inline-flex items-center rounded bg-slate-100 dark:bg-slate-850 px-2 py-0.5 text-[10px] font-bold text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-800">{{ $task->task_process }}</span>
                                @else
                                    -
                                @endif
                            </td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $task->specific_process ?: '-' }}</td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                            ...
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $task->company ?: '-' }}</td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $task->project?->name ?? 'N/A' }}</td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $task->deliverables ?: '-' }}</td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                            ...
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                                ...
                                @if(($task->comments_count ?? 0) > 0 && $task->latestComment)
                                    <div class="mt-1 rounded border border-slate-200/60 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-1.5">
                                        <p class="text-[10px] font-bold text-slate-800 dark:text-slate-200">Latest comment ({{ $task->comments_count }})</p>
                                        <p class="mt-0.5 line-clamp-2 text-[10px] text-slate-550 dark:text-slate-400">"{{ \Illuminate\Support\Str::limit($task->latestComment->body, 90) }}"</p>
                                        <p class="mt-0.5 text-[9px] text-slate-400 dark:text-slate-500">{{ $task->latestComment->user?->name ?? 'Unknown' }} • {{ $task->latestComment->created_at?->diffForHumans() }}</p>
                                    </div>
                                @endif
                            </td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                                ...
                                <div class="mt-1.5">
                                    <span class="inline-flex items-center rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide {{ $statusBadgeClass }}">
                                        {{ $workflowStatusLabel }}
                                    </span>
                                </div>
                            </td>
                            <td class="py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">
                                ...
                                @endphp
                                <span class="inline-flex items-center rounded px-2 py-0.5 text-[10px] font-bold {{ $roleClass }}">{{ ucwords(str_replace('_', ' ', $roleLabel)) }}</span>
                            </td>
                            <td class="whitespace-nowrap py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $task->date_received ? \Carbon\Carbon::parse($task->date_received)->format('M d, Y') : '-' }}</td>
                            <td class="whitespace-nowrap py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $task->date_started ? $task->date_started->format('M d, Y') : '-' }}</td>
                            <td class="whitespace-nowrap py-2.5 px-3 text-xs font-medium text-slate-800 dark:text-slate-100 leading-relaxed">{{ $dateFinish ? $dateFinish->format('M d, Y') : '-' }}</td>
                            <td class="whitespace-nowrap py-2.5 px-3 text-xs font-medium leading-relaxed {{ $isDelayed ? 'font-semibold text-amber-700 dark:text-amber-400' : 'text-slate-800 dark:text-slate-100' }}">
                                {{ $task->due_date ? $task->due_date->format('m-d-Y') : '-' }}
                                @if($isDelayed)
                                    <span class="ml-1.5 inline-flex items-center rounded bg-amber-100 dark:bg-amber-900/30 px-1.5 py-0.5 text-[10px] font-bold text-amber-800 dark:text-amber-300">Overdue {{ $overdueDays }} d</span>
                                @endif
                            </td>
                            <td class="whitespace-nowrap py-2.5 px-3 text-right text-xs">
                                <a href="{{ route('tasks.show', $task) }}" class="mr-1 inline-flex items-center gap-1 rounded border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-950/40 px-2 py-1 text-[10px] font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-100/80 dark:hover:bg-blue-900/50 transition-all duration-205 hover:-translate-y-0.5 shadow-sm">Open</a>
                                @if(auth()->user()?->isAdmin())
                                    <a href="{{ route('tasks.edit', $task) }}" class="mr-1 inline-flex items-center gap-1 rounded border border-yellow-100 dark:border-yellow-900/50 bg-yellow-50/50 dark:bg-yellow-950/40 px-2 py-1 text-[10px] font-bold text-yellow-600 dark:text-yellow-400 hover:bg-yellow-100/80 dark:hover:bg-yellow-900/50 transition-all duration-205 hover:-translate-y-0.5 shadow-sm">Edit</a>
                                @else
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-3 sm:flex-row bg-slate-50/50 dark:bg-slate-900/50 p-3 sm:p-4 rounded-xl border border-slate-200/40 dark:border-slate-800 backdrop-blur-md">
                <form method="GET" action="{{ url()->current() }}" class="flex items-center gap-1.5 text-xs text-slate-550 dark:text-slate-400">
                    ...
                    @endforeach
                    <span>Show</span>
                    <input type="number" name="per_page" value="{{ method_exists($tasks, 'perPage') ? $tasks->perPage() : 20 }}" min="1" max="100" 
    ...
    @if ($currentTab === 'done')
        ...
        @else
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-3 sm:flex-row bg-slate-50/50 dark:bg-slate-900/50 p-3 sm:p-4 rounded-xl border border-slate-200/40 dark:border-slate-800 backdrop-blur-md">
                <form method="GET" action="{{ url()->current() }}" class="flex items-center gap-1.5 text-xs text-slate-550 dark:text-slate-400">
                    @foreach(request()->except('per_page_done', 'done_page') as $key => $value)
                        @if(is_array($value))
                            @foreach($value as $k => $v)
                                <input type="hidden" name="{{ $key }}[]" value="{{ $v }}">
                            @endforeach
                    ...
                    @endforeach
                    <span>Show</span>
                    <input type="number" name="per_page_done" value="{{ method_exists($doneTasks, 'perPage') ? $doneTasks->perPage() : 10 }}" min="1" max="100" 
                           class="w-12 rounded-lg border border-slate-300 dark:border-slate-700 bg-white/70 dark:bg-slate-900/50 py-1 px-1.5 text-center text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all duration-150"
                           onchange="this.form.submit()">
                    <span>entries</span>
                </form>
