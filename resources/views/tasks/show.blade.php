...
<div class="space-y-4">
    <div class="relative overflow-hidden rounded-xl border border-slate-200/40 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/90">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{{ $task->title }}</h1>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Project: <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $task->project?->name ?? 'N/A' }}</span></p>
            </div>
            <a href="{{ route('tasks.list') }}" class="inline-flex items-center justify-center rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-200 hover:-translate-y-0.5">Back to Tasks</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div class="space-y-4 lg:col-span-2">
            <div class="rounded-xl border border-slate-200/40 bg-white/90 p-4 dark:border-slate-800 dark:bg-slate-900/90 shadow-sm">
                ...
                <div class="grid grid-cols-1 gap-x-4 gap-y-3 md:grid-cols-2">
                    <div>
                        ...
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ ucwords(str_replace('_', ' ', $task->status)) }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Priority</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ ucfirst($task->priority) }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Due</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->due_date ? $task->due_date->format('m-d-Y') : '-' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Date Received</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->date_received ? $task->date_received->format('M d, Y') : '-' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Date Started</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->date_started ? $task->date_started->format('M d, Y') : '-' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Task Process</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->task_process ?: '-' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Company/Client</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->company ?: '-' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Person-in-charge</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->team_in_charge ?: '-' }}</p>
                    </div>
                    <div>
                        ...
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                            @if($task->blockedByTask)
                                <a href="{{ route('tasks.show', $task->blockedByTask) }}" class="text-blue-650 dark:text-blue-400 hover:underline">{{ $task->blockedByTask->title }}</a>
                            @else
                                -
                            @endif
                        </p>
                    ...
                    <div>
                        ...
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->deliverables ?: '-' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-450 dark:text-slate-500">Project Owner</p>
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ $task->project?->project_owner ?: 'Sales (Sales Project)' }}</p>
                    </div>
                </div>
                @if($task->remarks)
            ...
            <div class="rounded-xl border border-slate-200/40 bg-white/90 p-4 dark:border-slate-800 dark:bg-slate-900/90 shadow-sm">
                ...
                @can('update', $task)
                    <form method="POST" action="{{ route('tasks.checklists.store', $task) }}" class="mb-3 flex flex-col gap-2 sm:flex-row">
                        @csrf
                        <input
                            type="text"
                            name="title"
                    ...
                    @error('title')
                        <p class="mb-3 text-xs text-red-650 font-semibold">{{ $message }}</p>
                    @enderror
                @endcan
                        ...
                        @foreach($task->checklistItems as $item)
                            <li class="flex items-center justify-between gap-2 rounded-lg border border-slate-200/40 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-2.5 py-1.5 text-xs">
                                <form method="POST" action="{{ route('tasks.checklists.toggle', [$task, $item]) }}" class="flex items-center gap-2">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="h-3.5 w-3.5 rounded border border-slate-300 dark:border-slate-700 {{ $item->is_completed ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600' : 'bg-white dark:bg-slate-900' }}"></button>
            ...
            <div class="rounded-xl border border-slate-200/40 bg-white/90 p-4 dark:border-slate-800 dark:bg-slate-900/90 shadow-sm">
                ...
                <form method="POST" action="{{ route('tasks.attachments.store', $task) }}" enctype="multipart/form-data" class="mb-3 flex flex-col gap-3 md:flex-row md:items-center">
                    @csrf
                ...
                @error('file')
                    <p class="mb-3 text-xs text-red-600 font-semibold">{{ $message }}</p>
                @enderror
                    ...
                    <ul class="divide-y divide-slate-100/50 dark:divide-slate-800/55">
                        @foreach($task->attachments as $attachment)
                            <li class="flex items-center justify-between py-2 text-xs">
                                <div>
                                    <p class="font-semibold text-slate-750 dark:text-slate-300">{{ $attachment->filename }}</p>
                                    <p class="text-[10px] text-slate-500 dark:text-slate-450">{{ number_format($attachment->size / 1024, 1) }} KB · {{ $attachment->created_at?->diffForHumans() }}</p>
                                </div>
                                <div class="flex items-center gap-1.5">
                                    @if(str_starts_with($attachment->mime_type, 'image/'))
                                        <button type="button" onclick="openImageModal('{{ route('attachments.view', $attachment->id) }}')" class="inline-flex items-center gap-1.5 rounded border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-950/40 px-2.5 py-0.5 text-[10px] font-semibold text-blue-600 dark:text-blue-400 hover:bg-blue-100/80 dark:hover:bg-blue-900/50 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">View</button>
                                    @endif
                                    <a href="{{ route('attachments.download', $attachment->id) }}" class="inline-flex items-center gap-1.5 rounded border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 px-2.5 py-0.5 text-[10px] font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">Download</a>
            ...
            <div class="rounded-xl border border-slate-200/40 bg-white/90 p-4 dark:border-slate-800 dark:bg-slate-900/90 shadow-sm">
                ...
                <form method="POST" action="{{ route('comments.store', $task) }}" class="mb-3 space-y-2">
                    @csrf
                    <textarea
                        name="body"
                        rows="3"
                        ...
                        @foreach($task->comments->sortByDesc('created_at')->take(20) as $comment)
                            <li class="rounded-lg border border-slate-200/40 bg-slate-50 dark:bg-slate-900/50 p-2.5 text-xs">
                                <div class="mb-1 flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-450">
                                    <span class="font-bold text-slate-800 dark:text-slate-200">{{ $comment->user?->name ?? 'Unknown user' }}</span>
                                ...
                                <p class="text-slate-700 dark:text-slate-350 mt-0.5 leading-normal">{{ $comment->body }}</p>
                            </li>
                        @endforeach
                    </ul>
                @endif
        ...
        <div class="space-y-4">
            <div class="rounded-xl border border-slate-200/40 bg-white/90 p-4 dark:border-slate-800 dark:bg-slate-900/90 shadow-sm">
                ...
                <div class="mb-3 rounded-lg border {{ ($hasCompletionDocument && !$hasIncompleteChecklists) ? 'border-emerald-200/40 dark:border-emerald-800/40 bg-emerald-50/50 dark:bg-emerald-950/20' : 'border-amber-200/40 dark:border-amber-800/40 bg-amber-50/50 dark:bg-amber-950/20' }} px-3 py-2 text-xs">
                    <div class="mb-1 flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            <p class="font-bold {{ ($hasCompletionDocument && !$hasIncompleteChecklists) ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-400' }}">
                                {{ ($hasCompletionDocument && !$hasIncompleteChecklists) ? 'Ready for review' : 'Requirements pending' }}
                            </p>
                            <div class="mt-1 space-y-0.5 text-[11px] font-medium {{ ($hasCompletionDocument && !$hasIncompleteChecklists) ? 'text-emerald-600 dark:text-emerald-500' : 'text-amber-600 dark:text-amber-500' }}">
                        ...
                        <span class="inline-flex shrink-0 items-center {{ $approvalStatusClass }}">
                            {{ $approvalStatusLabel }}
                        </span>
                    </div>
                </div>
