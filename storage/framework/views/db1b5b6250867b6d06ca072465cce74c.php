...
<div class="mx-auto max-w-7xl">
    <div class="mb-4 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
        ...
        <div class="flex gap-1.5">
            ...
            <a href="<?php echo e(route('supplier.export')); ?>" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 py-1.5 px-3 text-xs font-bold text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-200">
    ...
    <div class="rounded-xl border border-slate-200/40 bg-white/90 shadow-sm dark:border-slate-800 dark:bg-slate-900/90">
        <div class="border-b border-slate-200/40 p-3 dark:border-slate-800">
            <form method="GET" action="<?php echo e(route('supplier.index')); ?>" class="flex max-w-md items-center gap-2">
                <div class="relative flex-1">
                    <i data-lucide="search" class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"></i>
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search by name, category, status..." class="w-full rounded-lg border border-slate-300 bg-slate-50/70 py-1.5 pl-8 pr-3 text-xs focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900/50 dark:text-white">
                </div>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-850">
                    <?php $__empty_1 = true; $__currentLoopData = $suppliers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-850/50 transition-colors">
                            <td class="whitespace-nowrap px-3 py-2.5 font-semibold text-slate-900 dark:text-white text-xs"><?php echo e($item->name); ?></td>
                            <td class="px-3 py-2.5 text-xs">
                                ...
                                ?>
                                <select onchange="updateInlineStatus(this, <?php echo e($item->id); ?>, 'supplier')" class="text-[10px] font-bold rounded-full px-2 py-0.5 appearance-none border-0 cursor-pointer focus:ring-1 focus:ring-blue-500 <?php echo e($colorClass); ?>">
                                    ...
                                    <option value="ended" <?php echo e($item->status == 'ended' ? 'selected' : ''); ?> class="bg-white text-slate-900 dark:bg-slate-905 dark:text-white">Ended</option>
                                    ...
                                    <option value="confirmed/booked" <?php echo e($item->status == 'confirmed/booked' ? 'selected' : ''); ?> class="bg-white text-slate-900 dark:bg-slate-905 dark:text-white">Confirmed/Booked</option>
                                </select>
                            </td>
                            <td class="px-3 py-2.5 text-xs"><?php echo e($item->category); ?></td>
                            <td class="px-3 py-2.5 text-xs"><?php echo e($item->contact); ?></td>
                            ...
                            <td class="px-3 py-2.5 text-xs">
                                <?php if($item->proof_of_completion): ?>
                                    <a href="<?php echo e($item->proof_of_completion); ?>" target="_blank" class="text-blue-600 hover:underline font-semibold">View Link</a>
                                <?php endif; ?>
                            </td>
                            <td class="px-3 py-2.5 text-xs max-w-[200px] truncate" title="<?php echo e($item->remarks); ?>"><?php echo e($item->remarks); ?></td>
                            <td class="whitespace-nowrap px-3 py-2.5 text-right text-xs">
                                <div class="inline-flex gap-1">
                                    ...
                                    <button type="button" onclick="openDeleteModal(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->name)); ?>')" class="inline-flex items-center gap-1 rounded border border-red-100 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/40 px-2 py-1 text-[10px] font-bold text-red-650 dark:text-red-400 hover:bg-red-100/80 dark:hover:bg-red-900/50 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">Delete</button>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="border-t border-slate-200 dark:border-slate-850 p-3 sm:p-4 flex flex-col items-center justify-between gap-3 sm:flex-row bg-slate-50/50 dark:bg-slate-900/50 rounded-b-xl backdrop-blur-md">
            <form method="GET" action="<?php echo e(url()->current()); ?>" class="flex items-center gap-1.5 text-xs text-slate-550 dark:text-slate-400">
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <span>Show</span>
                <input type="number" name="per_page" value="<?php echo e($suppliers->perPage()); ?>" min="1" max="100" 
...
<div id="supplier-modal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-900/50 backdrop-blur-sm">
    <div class="flex min-h-full items-start justify-center p-4 py-10">
        <div class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
            ...
            <form id="supplier-form" method="POST" action="<?php echo e(route('supplier.store')); ?>">
                <?php echo csrf_field(); ?>
...
<div id="import-modal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-900/50 backdrop-blur-sm">
    <div class="flex min-h-screen items-center justify-center p-4">
        <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
            ...
            <form method="POST" action="<?php echo e(route('supplier.import')); ?>" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
                <div class="mb-6">
...
<script>
    const supplierUrl = "<?php echo e(route('supplier.index')); ?>";
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Local.Administrator\Herd\taskmanagement\resources\views/supplier/index.blade.php ENDPATH**/ ?>