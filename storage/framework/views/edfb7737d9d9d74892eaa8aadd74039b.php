...
<div class="space-y-6">
    <div class="relative overflow-hidden rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-md dark:shadow-none">
        ...
        <a href="<?php echo e(route('tasks.calendar')); ?>" class="rounded-full border border-emerald-300 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-100 dark:border-emerald-500/40 dark:bg-emerald-500/10 dark:text-emerald-300 dark:hover:bg-emerald-500/20">Open Calendar</a>
        </div>
    </div>
    ...
    <?php if(auth()->user() && in_array((string) data_get(auth()->user(), 'role', ''), ['manager', 'admin'], true)): ?>
        <div class="rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-md dark:shadow-none">
            ...
            <form method="POST" action="<?php echo e(route('holidays.store')); ?>" class="grid grid-cols-1 gap-3 md:grid-cols-3">
                <?php echo csrf_field(); ?>
    ...
    <div class="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/30 p-6 text-center text-amber-700 dark:text-amber-400 text-sm font-semibold">
        ...
        <a href="<?php echo e(route('tasks.calendar')); ?>" class="mt-4 inline-block rounded-full border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 px-4 py-2 text-sm font-semibold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/50">Open Calendar</a>
    </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Local.Administrator\Herd\taskmanagement\resources\views\holidays\index.blade.php ENDPATH**/ ?>