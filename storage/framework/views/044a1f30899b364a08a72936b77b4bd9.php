    ...
    <div class="relative w-full">
        ...
        <div class="mb-6 flex flex-wrap items-start justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-xs text-slate-400 dark:text-slate-400">As of <?php echo e($today->format('M d, Y')); ?></p>
            </div>
            <div class="flex gap-2">
                <a href="<?php echo e(route('email.shortcut', ['send_test' => 1, 'to' => auth()->user()?->email])); ?>" class="rounded-lg bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm">Send Test Email</a>
                ...
                <a href="<?php echo e(route('dashboard')); ?>" class="rounded-lg border border-slate-300 dark:border-white/20 bg-white dark:bg-white/5 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-white/10">Back to Dashboard</a>
            </div>
        </div>
        ...
        <details id="overdue-reminder-status" open class="mb-4 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-md">
            ...
            <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-slate-600 dark:text-slate-300 md:grid-cols-2">
                <p><span class="font-semibold text-slate-800 dark:text-slate-100">Last Run:</span> <?php echo e(!empty($overdueReminderLastRunAt) ? \Carbon\Carbon::parse($overdueReminderLastRunAt)->format('M d, Y h:i A') : 'Not yet recorded'); ?></p>
                <p><span class="font-semibold text-slate-800 dark:text-slate-100">Emails Sent:</span> <?php echo e($overdueReminderLastSentCount ?? 0); ?></p>
            </div>
            <div class="mt-3 flex flex-wrap gap-2 text-xs">
                ...
                <?php if(isset($toPreview) && $toPreview->isNotEmpty()): ?>
                    <a href="mailto:<?php echo e($toPreview->first()); ?>" class="rounded border border-sky-300 dark:border-sky-400/40 bg-sky-50 dark:bg-sky-500/10 px-3 py-1.5 font-medium text-sky-700 dark:text-sky-300 hover:bg-sky-100 dark:hover:bg-sky-500/20">Open Email App</a>
                <?php endif; ?>
            </div>
        </details>
                ...
                <div id="department-email-tabs" class="mt-3">
                    <div class="mb-3 flex flex-wrap gap-2" role="tablist" aria-label="Department Email Tabs">
                        <?php $__currentLoopData = $departmentEmailDirectory; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <button
                                type="button"
                                class="department-tab-btn rounded-full border px-3 py-1.5 text-xs font-semibold transition <?php echo e($index === 0 ? 'border-blue-300 dark:border-blue-400/40 bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-200' : 'border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'); ?>"
                    ...
                    <?php $__currentLoopData = $departmentEmailDirectory; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div
                            ...
                            class="department-tab-panel rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 p-3 text-xs text-slate-600 dark:text-slate-300 <?php echo e($index === 0 ? '' : 'hidden'); ?>"
                            role="tabpanel"
                        >
        ...
        <div class="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2">
            <?php $__currentLoopData = $sections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $config): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div id="<?php echo e($key); ?>-section" class="rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-md scroll-mt-24">
                    <div class="mb-3 flex items-center justify-between">
                        <h2 class="text-sm font-semibold text-slate-800 dark:text-slate-100"><?php echo e($config['label']); ?></h2>
                    ...
                    <?php else: ?>
                        <div class="space-y-2">
                            <?php $__currentLoopData = $config['tasks']->take(8); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a href="<?php echo e(route('tasks.show', $task)); ?>" class="block rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-600 <?php echo e($config['row']); ?>">
                                    <div class="flex items-center justify-between gap-2">
                                        <p class="font-medium text-slate-800 dark:text-slate-100"><?php echo e($task->title); ?></p>
                                        <span class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(optional($task->due_date)->format('M d, Y')); ?></span>
                                    </div>
                                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Project: <?php echo e($task->project->name ?? 'N/A'); ?></p>
                                </a>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Local.Administrator\Herd\taskmanagement\resources\views\emails\deadline-levels.blade.php ENDPATH**/ ?>