...
<div class="space-y-6">
    <div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold text-slate-800"><?php echo e($task->title); ?></h1>
                <p class="text-sm text-slate-500">Project: <?php echo e($task->project?->name ?? 'N/A'); ?></p>
            </div>
            <a href="<?php echo e(route('tasks.list')); ?>" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Back to Tasks</a>
        </div>
    </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 text-sm">
                    <div>
                        ...
                        <p class="font-medium text-slate-800"><?php echo e(ucwords(str_replace('_', ' ', $task->status))); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Priority</p>
                        <p class="font-medium text-slate-800"><?php echo e(ucfirst($task->priority)); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Due</p>
                        <p class="font-medium text-slate-800"><?php echo e($task->due_date ? $task->due_date->format('m-d-Y') : '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Date Received</p>
                        <p class="font-medium text-slate-800"><?php echo e($task->date_received ? $task->date_received->format('M d, Y') : '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Date Started</p>
                        <p class="font-medium text-slate-800"><?php echo e($task->date_started ? $task->date_started->format('M d, Y') : '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Task Process</p>
                        <p class="font-medium text-slate-800"><?php echo e($task->task_process ?: '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Company/Client</p>
                        <p class="font-medium text-slate-800"><?php echo e($task->company ?: '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Person-in-charge</p>
                        <p class="font-medium text-slate-800"><?php echo e($task->team_in_charge ?: '-'); ?></p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-medium text-slate-800"><?php echo e($task->deliverables ?: '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-slate-500">Project Owner</p>
                        <p class="font-medium text-slate-800"><?php echo e($task->project?->project_owner ?: 'Sales (Sales Project)'); ?></p>
                    </div>
                </div>
                <?php if($task->remarks): ?>
                        ...
                        <?php $__currentLoopData = $task->checklistItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li class="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
                                <form method="POST" action="<?php echo e(route('tasks.checklists.toggle', [$task, $item])); ?>" class="flex items-center gap-2">
                                    ...
                                    <button type="submit" class="h-4 w-4 rounded border border-slate-300 <?php echo e($item->is_completed ? 'bg-emerald-500' : 'bg-white'); ?>"></button>
                                    <span class="<?php echo e($item->is_completed ? 'text-slate-400 line-through' : 'text-slate-700'); ?>"><?php echo e($item->title); ?></span>
                                </form>
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('update', $task)): ?>
                    ...
                    <ul class="divide-y divide-slate-100">
                        <?php $__currentLoopData = $task->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attachment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li class="flex items-center justify-between py-3 text-sm">
                                <div>
                                    <p class="font-medium text-slate-700"><?php echo e($attachment->filename); ?></p>
                                    <p class="text-slate-500"><?php echo e(number_format($attachment->size / 1024, 1)); ?> KB · <?php echo e($attachment->created_at?->diffForHumans()); ?></p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <a href="<?php echo e(route('attachments.download', $attachment->id)); ?>" class="rounded border border-slate-300 px-3 py-1 text-slate-700 hover:bg-slate-50">Download</a>
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <?php $__currentLoopData = $task->comments->sortByDesc('created_at')->take(20); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li class="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
                                <div class="mb-1 flex items-center justify-between text-xs text-slate-500">
                                    <span class="font-medium text-slate-700"><?php echo e($comment->user?->name ?? 'Unknown user'); ?></span>
                                    <div class="flex items-center gap-2">
                                ...
                                <p class="text-slate-700"><?php echo e($comment->body); ?></p>
                            </li>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </ul>
                <?php endif; ?>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mb-4 rounded-xl border <?php echo e($hasCompletionDocument ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'); ?> px-4 py-3 text-sm">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <div>
                            <p class="font-semibold <?php echo e($hasCompletionDocument ? 'text-emerald-800' : 'text-amber-800'); ?>">
                                <?php echo e($hasCompletionDocument ? 'Completion document uploaded' : 'No completion document yet'); ?>

                            </p>
                            <p class="mt-1 <?php echo e($hasCompletionDocument ? 'text-emerald-700' : 'text-amber-700'); ?>">
                                <?php echo e($hasCompletionDocument ? 'Attachment(s) are available in Task Attachments.' : 'Upload at least one proof file before requesting review.'); ?>
                ...
                <?php if($canUpdateTaskStatus): ?>
                    <div class="grid grid-cols-1 gap-2">
                        <form method="POST" action="<?php echo e(route('tasks.status.update', $task)); ?>">
                            <?php echo csrf_field(); ?>
                        ...
                        <button
                            type="button"
                            class="w-full rounded-lg px-3 py-2 text-sm font-medium text-white <?php echo e($approvalButtonClass); ?>"
                            disabled
                        >
                            <?php echo e($approvalButtonLabel); ?>

                        </button>

                        <?php if($task->status === 'for_review' && $canApproveNow): ?>
                            <form method="POST" action="<?php echo e(route('tasks.status.update', $task)); ?>">
                                <?php echo csrf_field(); ?>
