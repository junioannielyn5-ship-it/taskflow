...
<div class="mx-auto max-w-7xl">
    <div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Stock Out (Dispatch)</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Record items distributed or dispatched to projects/clients.</p>
        </div>
        <div class="flex gap-2">
            <button type="button" onclick="openAddModal()" class="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-orange-700">
            ...
            <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                <thead class="bg-slate-50 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:bg-slate-700/50 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                    <tr>
                        ...
                        <th class="px-6 py-4">Quantity Out</th>
                        <th class="px-6 py-4">Reference (Project/Client)</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    <?php $__empty_1 = true; $__currentLoopData = $transactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $txn): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                            ...
                            <td class="px-6 py-4 font-medium text-orange-600">-<?php echo e($txn->quantity); ?></td>
                            <td class="px-6 py-4"><?php echo e($txn->reference ?? '-'); ?></td>
    ...
    <div class="flex min-h-full items-start justify-center p-4 py-10">
        <div class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Record Stock Out</h3>
            ...
            <form method="POST" action="<?php echo e(route('inventory.transactions.store')); ?>">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Date Dispatched</label>
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Quantity to Dispatch</label>
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Reference (Project / Client)</label>
                        <input type="text" name="reference" placeholder="e.g. Project Alpha" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white">
                    </div>
                    <div class="md:col-span-2">
                        <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Remarks / Reason</label>
                ...
                <div class="mt-6 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-700 pt-4">
                    ...
                    <button type="submit" class="rounded-xl bg-orange-600 px-6 py-2 text-sm font-medium text-white hover:bg-orange-700 shadow-sm shadow-orange-500/30">Save Stock Out</button>
                </div>
            </form>
        </div>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Local.Administrator\Herd\taskmanagement\resources\views/inventory/transactions/stock_out.blade.php ENDPATH**/ ?>