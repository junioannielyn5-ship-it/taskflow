                ...
                <form action="<?php echo e(route('profile.update')); ?>" method="POST" class="space-y-4">
                    ...
                    <div x-data="{ uploading: false }" class="flex items-center gap-4 mb-6">
                        <div class="relative group">
                            <?php if(auth()->user()->profile_photo_path): ?>
                                <img src="<?php echo e(asset('storage/' . auth()->user()->profile_photo_path)); ?>" alt="Profile" class="h-16 w-16 rounded-full object-cover ring-2 ring-indigo-500/30 shadow-md">
                            <?php else: ?>
                            ...
                            <?php if(auth()->user()->profile_photo_path): ?>
                                <form action="<?php echo e(route('profile.photo.destroy')); ?>" method="POST" class="absolute -top-2 -right-2">
                                    <?php echo csrf_field(); ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="<?php echo e(old('name', auth()->user()->name)); ?>" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none transition-all">
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div>
                            ...
                            <input type="text" value="<?php echo e(auth()->user()->roleLabel()); ?>" disabled class="w-full bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-500 dark:text-slate-400 cursor-not-allowed">
                        </div>
                    </div>

                    <div class="w-full md:w-1/2 mt-4">
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Email Address</label>
                        <input type="email" name="email" value="<?php echo e(old('email', auth()->user()->email)); ?>" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none transition-all">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
            ...
            <div class="p-6">
                <form action="<?php echo e(route('user-password.update')); ?>" method="POST" class="space-y-4 max-w-md w-full">
                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Local.Administrator\Herd\taskmanagement\resources\views\settings\profile.blade.php ENDPATH**/ ?>